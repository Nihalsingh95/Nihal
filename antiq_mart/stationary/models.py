...
class Antique(models.Model):
    ...
    antique_image =models.ImageField(upload_to='antique_images')  # As we are using image field we have to intall 'pillow'. And we have to Define MEDIA_URL in settings.py file so that all folder should save in media directory
...
class Cart(models.Model):
    ...
    product = models.ForeignKey(Antique, on_delete=models.CASCADE)     #Each cart instance is associated with a single pet product. Similarly, the on_delete=models.CASCADE argument means that if the referenced Pet object is deleted, the corresponding Cart instances associated with that product will be deleted.
